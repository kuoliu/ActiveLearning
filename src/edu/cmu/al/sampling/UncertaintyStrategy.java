	...
	public HashSet<String> sampling(int k) {
		...
		try {
			while (rs.next()) {
				...
				if (!isLabled(prod_id)) {
					if (getPredictResult(prod_id) >= 0.7) {
						positive.put(positiveID++, prod_id);
					} else if (getPredictResult(prod_id) >= 0.4
							&& getPredictResult(prod_id) <= 0.6) {
