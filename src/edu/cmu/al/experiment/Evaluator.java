...
public class Evaluator implements EvaluatorInterface {
	...
	public void evaluateClassification() {
		...
		getFalsePos();
		getFalseNeg();
		// System.out.println(TRUE_POS + "\t" + TRUE_NEG + "\t" + FALSE_POS +
		// "\t" + FALSE_NEG);
	...
	public void getTruePos() {
		...
		String sql = "select count(*) from " + Configuration.getFeatureTable() + " R," + Configuration.getPredictTable() + " P "
				+ "where R.product_id = P.product_id and P.predict_result = 1 and R.f2 >=" + ScoreDefine.posSocre;
		TRUE_POS = SqlManipulation.queryInt(sql);
	}

	public void getTrueNeg() {
		String sql = "select count(*) from " + Configuration.getFeatureTable() + " R," + Configuration.getPredictTable() + " P "
				+ "where R.product_id = P.product_id and P.predict_result = 0 and R.f2 <" + ScoreDefine.posSocre;
